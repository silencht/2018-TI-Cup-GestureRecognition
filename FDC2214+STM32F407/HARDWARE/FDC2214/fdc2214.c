...
u32 FCD2214_ReadCH(u8 index) 
{
	...
	{
		case 0:
		  result = FDC_Read(DATA_CH0)&0x0FFF;  //读高位， 12+16=28bit
		  result = (result<<16)|(FDC_Read(DATA_LSB_CH0));//高位左移或低位得最终结果值，下同
			break;
		case 1:
...
u8 FDC2214_Init(void)
{
	...
	FDC_GPIO_Init();//可能是原本打算配置ADDR与SD脚的函数，现在为空，保留，直接ADDR与SD硬件接地
	FDC_IIC_Init();//模拟IIC初始化
	res=FDC_Read(MANUFACTURER_ID);//读取制造商ID号（实际寄存器地址为0X7E，实际应读取值为5549），测试设备读取是否正常
	
	ID_FDC2214 = FDC_Read(DEVICE_ID);//读取设备ID号，并幅值给ID号变量
	...
	if(res==0x5449)//如果设备正确
	{
		...
		Set_FDC2214(RCOUNT_CH2,0x34,0xFB);//设置为34FB的原因是，手册17页公式（5） tcx=（0x34FB*16+4）/21.7M=0.01s，采样间隔10ms
		Set_FDC2214(RCOUNT_CH3,0x34,0xFB);
		
		Set_FDC2214(SETTLECOUNT_CH0,0x00,0x0A);//转换之前的稳定时间（T=(SETTLECOUNT_CHx*16)/Frefx）
		Set_FDC2214(SETTLECOUNT_CH1,0x00,0x0A);//（需大于）推荐最小值0A，源程序设置为001B    手册推荐值 0A
		Set_FDC2214(SETTLECOUNT_CH2,0x00,0x0A);//设置001B原因：手册16页公式（3）tsx=0x001B*16/21.7M=0.016ms   【0x1B(2)=27(10)】
		Set_FDC2214(SETTLECOUNT_CH3,0x00,0x0A);
		
		Set_FDC2214(CLOCK_DIVIDERS_C_CH0,0x20,0x02);//选择在0.01MHz ~ 10MHz的传感器输入频率		
		Set_FDC2214(CLOCK_DIVIDERS_C_CH1,0x20,0x02);//Frefx = Fclk/CHx_DIVIDER = 43.4MHz/2(2分频)=21.7Mhz   //内部Fclk实际可能为43.3mhz左右，详情page10
		Set_FDC2214(CLOCK_DIVIDERS_C_CH2,0x20,0x02);//CHx_FIN_SEL=2		CHx_REF_DIVIDER=2;
		Set_FDC2214(CLOCK_DIVIDERS_C_CH3,0x20,0x02);
		
		Set_FDC2214(DRIVE_CURRENT_CH0,0xF8,0x00);//0.146ma（传感器时钟建立+转换时间的驱动电流）datasheet page20 and 36
		Set_FDC2214(DRIVE_CURRENT_CH1,0xF8,0x00);//控制传感器驱动电流，使传感器振荡峰值位于1.2V~1.8V之间 原始设置0x78
		Set_FDC2214(DRIVE_CURRENT_CH2,0xF8,0x00);
		Set_FDC2214(DRIVE_CURRENT_CH3,0xF8,0x00);
...
float Cap_Calculate(u8 chx)  //采集计算电容值
{
	float Cap;
